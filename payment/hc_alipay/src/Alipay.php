...
class Alipay {
  ...
  public function __construct($alipay_config) {
    ...
    $this->_core = $core = new AlipayCore;
    $this->_md5  = $md5  = new AlipayMd5;
  }
  ...
  public function buildRequestMysign($para_sort) {
    ...
    switch (strtoupper(trim($this->alipay_config['sign_type']))) {
      case "MD5":
        $mysign = $this->_md5->md5Sign($prestr, $this->alipay_config['key']);
        break;
      default:
        $mysign = "";
    }
   ...
   */
	public function buildRequestParaToString($para_temp) {
		...
		$request_data = $this->_core->createLinkstringUrlencode($para);
  ...
  public function create_url($param) {
		//待请求参数数组
		$para = $this->buildRequestPara($param);
    $request_data = $this->buildRequestParaToString($para);
   ...
   */
	public function buildRequestHttp($para_temp) {
		...
		//远程获取数据
		$sResult = $this->_core->getHttpResponsePOST($this->alipay_gateway_new, $this->alipay_config['cacert'],$request_data,trim(strtolower($this->alipay_config['input_charset'])));
   ...
   */
	public function buildRequestHttpInFile($para_temp, $file_para_name, $file_name) {
		...
		//远程获取数据
		$sResult = $this->_core->getHttpResponsePOST($this->alipay_gateway_new, $this->alipay_config['cacert'],$para,trim(strtolower($this->alipay_config['input_charset'])));
