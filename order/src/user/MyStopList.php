...
/**
 * @file
 * Contains \Drupal\order\user\MyStopList.
 */
...
class MyStopList {
  ...
  public function buildForm(array $qys = array()) {
    $tral_server = entity_load_multiple_by_properties('hostclient', array('client_uid' => \Drupal::currentUser()->id(), 'status' => 4));
    if($tral_server) {
      $form['stop_table'] = array(
        '#type' => 'table',
        '#header' => $this->buildHeader(),
        '#empty' => t('No server')
      );
      foreach($tral_server as $key =>$hostclient) {
        $form['stop_table']['#rows'][$key] = $this->buildRow($hostclient, $qys);
      }
      return $form;
    }
    return array();
