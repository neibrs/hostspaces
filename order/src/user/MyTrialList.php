...
/**
 * @file
 * Contains \Drupal\order\user\MyTrialList.
 */
...
class MyTrialList {
  ...
  public function buildForm(array $qys = array()) {
    $tral_server = entity_load_multiple_by_properties('hostclient', array('client_uid' => \Drupal::currentUser()->id(), 'trial' => 1));
    if($tral_server) {
      $form['trial_table'] = array(
        '#type' => 'table',
        '#header' => $this->buildHeader(),
        '#empty' => t('No server')
      );
      foreach($tral_server as $key =>$hostclient) {
        $form['trial_table']['#rows'][$key] = $this->buildRow($hostclient, $qys);
      }
      return $form;
    }
    return array();
  ...
  private function getOperations($hostclient) {
    ...
    $handle_info = $hostclinet_service->getHandleInfoTrial($hostclient->id());
    $order = entity_load('order', $handle_info->handle_order_id);
    if($order->getSimpleValue('status') == 0) {
      $operations['payment'] = array(
        'title' => $this->t('Payment'),
        'url' => new Url('user.order.payment', array('order'=>$order->id()))
      );
    }
    return $operations;
  }
