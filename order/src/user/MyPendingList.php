...
/**
 * @file
 * Contains \Drupal\order\user\MyPendingList.
 */
...
class MyPendingList {
  ...
  public function buildForm(array $qys = array()) {
    $tral_server = entity_load_multiple_by_properties('hostclient', array('client_uid' => \Drupal::currentUser()->id(), 'trial' => 0, 'status' => 0));
    if($tral_server) {
      $form['pending_table'] = array(
        '#type' => 'table',
        '#header' => $this->buildHeader(),
        '#empty' => t('No server')
      );
      foreach($tral_server as $key =>$hostclient) {
        $form['pending_table']['#rows'][$key] = $this->buildRow($hostclient, $qys);
      }
      return $form;
    }
    return array();
